# Project

## Product definition

Build a deeply embodied craftsmanship and management RPG centered on premium cigar production. The canonical creative direction is defined in [`VISION.md`](VISION.md) and indexed by [`docs/design/README.md`](docs/design/README.md).

## Core pillars

- **Craftsmanship:** player knowledge and deliberate choices determine quality.
- **Growth without displacement:** delegation expands choice without making personal craft obsolete.
- **Patience:** meaningful processes unfold over days, seasons, and years without empty waiting.
- **Provenance:** every meaningful lot and cigar carries its agricultural and process history.
- **Legacy:** the finca, inventory, artisans, relationships, and reputation tell a persistent story.

## Target

- Unity 6 LTS (6000.5.4f1)
- C#
- Universal Render Pipeline
- Windows via Steam

## Directional constraints

- The finca is an intimate, physically represented production pipeline rather than an acreage-maximization map.
- Growing estate tobacco is important, but relationships with other growers are required for a broad blending vocabulary.
- Named artisans reproduce proven standards while the player retains access to hands-on work at every stage of progression.
- Reputation is audience-specific and emerges from products, promises, relationships, and conduct.
- Tobacco is an agricultural and artisanal material rather than a generic crafting ingredient.

## Current project focus

The cigar-craft loop now has canonical direction from leaf processing through tasting and revision. Numerical tuning must preserve those documents and must not introduce a universal cigar-quality score.

The market and economy now have canonical direction from crop and inventory allocation through release, payment, sell-through, and recovery from financial distress.

Characters and relationships now have canonical direction: multidimensional bonds, consequential memory, explicit commitments, artisan agency, hybrid authored and systemic content, and close friendship without romance mechanics.

Facilities and estate progression now have canonical direction: a neglected but viable opening, a semi-authored compact finca, capability-focused improvements, soft capacity, readable environmental risk, game-first abstraction, and visible growth without factory sprawl.

Knowledge and long-term progression now have canonical direction: evidence-based understanding, specific technique, institutional house standards, contextual access, persistent history, and a founder-centered legacy without universal levels or required succession.

Setting and narrative now have canonical direction: a fictional valley and town in real northern Nicaragua, an undated contemporary period, credible founder backgrounds, a financed acquisition, English text without voice-over, a three-layer narrative, and required cultural research and review.

Time and pacing now have canonical direction: three daily blocks, a 96-day prototype year, climate periods rather than temperate seasons, bundled travel, safe advancement, compressed processes, and persistent monthly and annual history.

The pre-implementation design foundation is complete. Milestone 1 is implemented: the player can inhabit the finca route, distinguish free inspection from focused work, advance the approved calendar, encounter scheduled checkpoints, inspect provenance-aware lots, and save or restore player, calendar, estate, and inventory state.

Milestone 2 is implemented and test-validated. The player can record an intent, choose an authored composition, construct a study cigar, inspect hard evidence, conduct a staged perspective tasting, diagnose a likely cause, preserve a recipe revision, and compare two versions without a universal quality score. The finca now also has a handmade tobacco-specific presentation pass.

Milestone 3A's spatial and guidance foundation is implemented and automated-test validated. The runtime finca separates the field, curing, fermentation, storage, workshop, aging, office, home, and tasting spaces across a readable estate. Opening orientation, state-driven objectives, contextual explanations, explicit costs, and an in-game craft glossary replace the previous opaque player flow.

The first hands-on review rejected both performance and visual quality: the initial build multiplied an overly large procedural renderer count through shadow-casting point lights, and the textured primitive architecture still read as graybox art. The replacement candidate now uses imported Blender-authored architecture, terrain, cigar-production props, CC0 PBR materials and vegetation, a rural HDR sky, and measured LOD/culling. It passes automated and screenshot review at 404 renderers, 145 LOD groups, eight visible Unity primitive meshes, and zero shadowed point lights. Milestone 3B remains blocked until this candidate passes another hands-on play review.
